...
stationarity_transposed <- data.frame(
  Test = c("ADF", "KPSS"),
  log_price = c(round(adf_log, 4), round(kpss_log, 4)),
  diff_log_price = c(round(adf_diff, 4), round(kpss_diff, 4))
)

kbl(
  stationarity_transposed,
  caption = "Stationarity Test Results for Log Price Series",
  col.names = c("Test Type", "$r_t = \\log(P_t)$", "$r_t = \\log(P_t) - \\log(P_{t-1})$")
) %>%
...
ggsave("figures/forecast_plot.png", plot = forecast_plot, width = 10, height = 5, dpi = 300)
